...
constexpr HardwareDriverResult notInitialized() {
  return HardwareDriverResult{HardwareDriverStatus::NotInitialized,
                              "PCA9685 servo driver is not initialized."};
}

constexpr HardwareDriverResult invalidChannel() {
  return HardwareDriverResult{HardwareDriverStatus::InvalidChannel,
                              "PCA9685 channel is outside 0..15."};
}

constexpr HardwareDriverResult invalidPwmValue() {
  return HardwareDriverResult{HardwareDriverStatus::InvalidPwmValue,
                              "PWM value is outside the PCA9685 12-bit range."};
}

constexpr HardwareDriverResult driverBeginFailed() {
  return HardwareDriverResult{HardwareDriverStatus::DriverBeginFailed,
                              "PCA9685 driver begin failed."};
}

}  // namespace

Pca9685ServoDriver::Pca9685ServoDriver(
    const Pca9685ServoDriverConfig &config)
    : config_(config),
      pwm_driver_(config.i2c_address),
      initialized_(false) {}
...
HardwareDriverResult Pca9685ServoDriver::write(
    const common::JointPwmState &state) {
  if (!initialized_) {
...
HardwareDriverResult Pca9685ServoDriver::writeChannel(uint8_t channel,
                                                      uint16_t pwm_value) {
