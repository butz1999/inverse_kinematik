...
* Umgang mit `OE`: Der `OE`-Pin des `PCA9685` wird softwareseitig über `GPIO6` geschaltet. Das Signal ist active-low; `HIGH` deaktiviert die PWM-Ausgänge, `LOW` gibt sie frei. Der Servo-Treiber sendet beim Begin einen PCA9685 Software Reset Call über die I2C-General-Call-Adresse, setzt `MODE2` explizit auf `0x04` (`OUTDRV=1`, `INVRT=0`, `OUTNE[1:0]=00`), setzt die PWM-Frequenz, schreibt den initialen `JointPwmState` und gibt die Ausgänge anschließend frei. Die Firmware setzt `OE` beim Boot zunächst auf `HIGH`; der Servo-Treiber gibt die Ausgänge im Bootablauf nach erfolgreichem `begin()` frei. Damit die Ausgänge auch während eines ESP32-Resets sicher deaktiviert bleiben, soll `OE` zusätzlich hardwareseitig mit einem Pull-up gegen `3.3V` versehen werden.
* Beobachtetes `OE`-Verhalten im Bring-up: Das `OE`-Signal selbst verhält sich elektrisch wie erwartet (`HIGH` nach Reset, `LOW` nach Init). Trotzdem wurde an den PCA9685-Ausgängen bei `OE = HIGH` weiterhin eine Spannung von ungefähr `1.2V` gemessen, die zu zuvor gesetzten PWM-Werten passte. Die Ursache ist aktuell nicht geklärt. Bis zur Klärung darf `OE` daher nicht als vollständige Leistungs- oder Spannungstrennung der Servo-Ausgänge verstanden werden, sondern nur als Deaktivierungssignal des PCA9685-Ausgangstreibers gemäß Datenblatt und Registerkonfiguration.
