...
void test_target_pose_validation_rejects_unreachable_position()
{
  const common::TargetPose pose{1000.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F};
  const auto result = robotics::validateTargetPose(pose, robotics::defaultRobotModel());

  TEST_ASSERT_FALSE(result.ok);
  TEST_ASSERT_EQUAL(robotics::ValidationStatus::TargetPoseOutOfWorkspace, result.status);
  TEST_ASSERT_EQUAL_STRING("position_mm", result.field_name.c_str());
}
...
int main(int argc, char **argv)
{
  ...
  RUN_TEST(test_target_pose_validation_rejects_unreachable_position);
